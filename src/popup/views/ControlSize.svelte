<script lang="ts">
  ...
  import type { AutoResize, Size } from "../../types";

  export let sizeVideo: Size = initial.size;
  export let isResizeVideo: AutoResize = initial.autoResize;
  ...
  function resizeVideo(e) {
    switch (e.key) {
      case "Enter": {
        const sizeNew: `${Size}` = e.target.firstElementChild.dataset.videoSize;
        sizeVideo = Number(sizeNew) as Size;
      }
        break;
